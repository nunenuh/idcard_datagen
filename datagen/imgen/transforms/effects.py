...
"""
From
https://github.com/UjjwalSaxena/Automold--Road-Augmentation-Library/blob/master/Automold.py
"""
...
__all__ = ['add_shadow', 'add_sun_flare', 'add_fog', 
           'add_snow', 'add_rain', 'add_gravel',
           'add_speed', 'add_autumn',]
...
def hls(image,src='RGB'):
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            eval('image_HLS.append(cv.cvtColor(img,cv.COLOR_'+src.upper()+'2HLS))')
    else:
        image_HLS = eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2HLS)')
    return image_HLS
...
def hsv(image,src='RGB'):
    ...
    if(is_list(image)):
        ...
        image_list=image
        for img in image_list:
            eval('image_HSV.append(cv.cvtColor(img,cv.COLOR_'+src.upper()+'2HSV))')
    else:
        image_HSV = eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2HSV)')
    return image_HSV
...
def bgr(image, src='RGB'):
    ...
    if(is_list(image)):
        ...
        image_list=image
        for img in image_list:
            eval('image_BGR.append(cv.cvtColor(img,cv.COLOR_'+src.upper()+'2BGR))')
    else:
        image_BGR= eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2BGR)')
    return image_BGR
...
def rgb(image, src='BGR'):
    ...
    if(is_list(image)):
        ...
        image_list=image
        for img in image_list:
            eval('image_RGB.append(cv.cvtColor(img,cv.COLOR_'+src.upper()+'2RGB))')
    else:
        image_RGB= eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2RGB)')
    return image_RGB
...
def red(image,src='BGR'):
    ...
    if(is_list(image)):
        ...
        image_list=image
        for img in image_list:
            i= eval('cv.cvtColor(img,cv.COLOR_'+src.upper()+'2RGB)')
            image_red.append(i[:,:,0])
    else:
        image_red= eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2RGB)[:,:,0]')
    return image_red
...
def green(image,src='BGR'):
    ...
    if(is_list(image)):
        ...
        image_green= eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2RGB)[:,:,1]')
    return image_green
...
def blue(image,src='BGR'):
    ...
    if(is_list(image)):
        ...
        image_blue= eval('cv.cvtColor(image,cv.COLOR_'+src.upper()+'2RGB)[:,:,2]')
    return image_blue
...
def rain_process(image,slant,drop_length,drop_color,drop_width,rain_drops):
    ...
    for rain_drop in rain_drops:
        cv.line(image_t,(rain_drop[0],rain_drop[1]),(rain_drop[0]+slant,rain_drop[1]+drop_length),drop_color,drop_width)
    image= cv.blur(image_t,(7,7)) ## rainy view are blurry
...
def add_fog(image, fog_coeff=-1):
    ...
    if(is_list(image)):
        ...
        image = cv.blur(image ,(hw//10,hw//10))
        image_RGB = image
...
def apply_motion_blur(image,count):
    ...
    while(i<=imshape[1]):
        image_t[:,i:,:] = cv.filter2D(image_t[:,i:,:], -1, kernel_motion_blur)
        image_t[:,:imshape[1]-i,:] = cv.filter2D(image_t[:,:imshape[1]-i,:], -1, kernel_motion_blur)
