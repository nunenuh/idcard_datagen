...
class RandomResize(object):
    ...
    def __call__(self, image: np.ndarray, boxes: np.ndarray) -> Tuple:
        pass
...
class AugmentGenerator(object):
    ...
    def __call__(self, background_image, foreground_image, boxes: np.ndarray = None):
        ...
        nfgdH, nfgdW = nfgd_size = mops.scale_size_ratio(back_size, frgd_size, ratio=self.scale_ratio)
        xmin, ymin, xmax, ymax = xybox = mops.random_safe_box_location(back_size, nfgd_size)
        ...
        frgd_segment_image = imops.image_selection(frgd_base_image, val=0)

        segment_canvas = imops.create_canvas(back_size)
        segment_image = imops.join2image_withcoords(frgd_segment_image, segment_canvas, xybox)

        overlay_canvas = imops.create_canvas((bgH, bgW, 4), dtype=np.uint8)
        overlay_image = imops.join2image_withcoords(frgd_base_image, overlay_canvas, xybox)
        composite_image = imops.composite2image(background_image, overlay_image)

        boxes = F.boxes_reorder(boxes)
